...
**Step 2 — Read chat.md**: Read `chat.md` from line `chat.executor.lastReadLine`
(stored in `.ralph-state.json`). Parse all messages after that line. Update
`lastReadLine` atomically after reading.

**Step 2b — CRITICAL: Read task_review.md for FAIL/WARNING signals**
BEFORE delegating ANY task, the coordinator MUST ALSO read `$SPEC_PATH/task_review.md`.
This is NOT optional and NOT dependent on chat.md signals.

- Parse ALL FAIL entries in task_review.md
- Parse ALL WARNING entries in task_review.md
- Check if current taskIndex is marked FAIL
- Check if any pending/future taskIndex is marked FAIL

**FAIL Signal Handling** (from task_review.md):
| Scenario | What coordinator does |
|----------|----------------------|
| **Current task (taskIndex) is marked FAIL** | DO NOT advance taskIndex. Add FIX task BEFORE delegating next task. Log to `.progress.md`: "REVIEWER FAIL on task $taskIndex — adding fix task". |
| **Previous task is marked FAIL and not yet fixed** | DO NOT advance. Add FIX task for the FAIL task first. |
| **Future task is marked FAIL** | When reaching that task, DO NOT advance. Add FIX task. |
| **task_review.md exists but no FAIL entries** | Proceed normally after logging: "task_review.md checked — no FAILs". |

**WARNING Signal Handling** (from task_review.md):
| Scenario | What coordinator does |
|----------|----------------------|
| **Current task is marked WARNING** | Note in `.progress.md` but may proceed. Do NOT block. |
| **Previous task has WARNING** | Proceed normally. Log to `.progress.md`: "WARNING on task N noted but not blocking". |

**Step 3 — Update lastReadLine**: After reading chat.md, update state atomically:
```bash
...
**Step 4 — Apply chat.md signal rules** (process all new messages top to bottom):
...
**Step 6 — After task completes**: After receiving TASK_COMPLETE and passing all 4
...
**Layer 2b: Verification Claim Integrity (MANDATORY — ANTI-FABRICATION)**

This layer catches when the executor FABRICATES verification results (claims commands passed when they did not, or reports false output).

**Rule: NEVER trust pasted verification output from spec-executor. ALWAYS run the verify command independently.**

For EVERY task that reports a verify command result (e.g., "ruff check → All checks passed", "pytest → 1371 passed", "grep → VE0_PASS"):
...
**Why this layer exists**: In the fix-emhass-sensor-attributes spec (2026-04-09), the spec-executor claimed "ruff check → All checks passed" when 72 errors existed, and claimed "1371 passed, 100.00% coverage" when tests were actually failing. The coordinator accepted both claims without independent verification, advancing 5+ tasks on false premises.
...
3. Verification claims match independent command execution (NO FABRICATION — Layer 2b)
...
After all 4 verification layers pass:
