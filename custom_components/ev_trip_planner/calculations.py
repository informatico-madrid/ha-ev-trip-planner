...
def calculate_multi_trip_charging_windows(
    ...
    for idx, (trip_departure_time, trip) in enumerate(trips):
        ...
        if window_start is not None and _ensure_aware(window_start) > _ensure_aware(
            trip_departure_time
        ):
        ...
        previous_arrival = _ensure_aware(trip_arrival) + timedelta(
        ...
        results.append(
            {
                ...
                "soc_cap_raw": round(soc_caps[original_idx], 2)
                if soc_caps is not None and original_idx < len(soc_caps)
                else 100.0,
