...
| ID | Veredicto | Razón |
|----|-----------|-------|
| C1 | ❌ FP con matiz | Python 3.12+ soporta f-strings anidados. HA 2024.1+ requiere 3.12. FP de Pylint. |
| C7 | ❌ FP | exists_fn ya existe (commit b390768). CI check era estado anterior. |
| C8-original | ❌ FP | `async_register_panel` desde services se llama via async. Bug real está en panel_custom.py standalone. |
| M1 | ❌ FP | Solo UNA clase TripPlannerCoordinator en coordinator.py. |
| M2 | ⚠️ Parcial | Bajo impacto. |
| m2, m9, m12-m15 | ❌ FP | Ver tabla de menores arriba. |
