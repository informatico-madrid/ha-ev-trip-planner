...
- [ ] 3.4 E2E test: Panel loads with vehicle ID extracted
  - **Do**: Create Playwright test that opens panel URL and verifies vehicle ID in header
  - **Files**: tests/e2e/test-panel-vehicle-id.spec.ts
  - **Done when**: Panel shows correct vehicle ID in header regardless of URL pattern
  - **Verify**: `npx playwright test tests/e2e/test-panel-vehicle-id.spec.ts`
  - **Commit**: `test(panel): add E2E test for vehicle ID extraction`
  - _Requirements: AC-1.1, AC-1.2_

- [ ] 3.5 E2E test: Trips load from trip_list service
  - **Do**: Create Playwright test that verifies trips appear in panel after service call
  - **Files**: tests/e2e/test-panel-trips.spec.ts
  - **Done when**: Panel displays trips with correct counts
  - **Verify**: `npx playwright test tests/e2e/test-panel-trips.spec.ts`
  - **Commit**: `test(panel): add E2E test for trip loading`
  - _Requirements: AC-2.1, AC-2.4_

- [ ] 3.6 E2E test: Sensor values display with proper formatting
  - **Do**: Verify percentages show 1 decimal, energy shows 2 decimals, distance shows 1 decimal
  - **Files**: tests/e2e/test-panel-sensors.spec.ts
  - **Done when**: All sensor values display with correct precision
  - **Verify**: `npx playwright test tests/e2e/test-panel-sensors.spec.ts`
  - **Commit**: `test(panel): add E2E test for sensor value formatting`
  - _Requirements: AC-3.2_

- [ ] 3.7 E2E test: Unavailable sensors show "N/A"
  - **Do**: Verify sensors with unavailable state show "N/A" instead of being hidden
  - **Files**: tests/e2e/test-panel-n-a.spec.ts
  - **Done when**: All sensors visible, unavailable ones show "N/A"
  - **Verify**: `npx playwright test tests/e2e/test-panel-n-a.spec.ts`
  - **Commit**: `test(panel): add E2E test for N/A sensor display`
  - _Requirements: AC-3.1_

- [ ] 3.8 [VERIFY] Quality checkpoint: all tests pass
  - **Do**: Run all unit and E2E tests
  - **Verify**: `npx playwright test --pass-with-no-tests`
...
- [ ] 5.3 E2E verification: Panel works end-to-end
  - **Do**: Run E2E tests against real Home Assistant instance
  - **Verify**: `HA_URL=http://ha:8123 npx playwright test tests/e2e/`
  - **Done when**: All E2E tests pass on real HA instance
  - **Commit**: None

- [ ] 5.4 VE1 [VERIFY] E2E startup: start test environment
  - **Do**:
    1. Start test HA instance if needed: `docker-compose up -d home-assistant`
    2. Wait for HA ready: `for i in $(seq 1 60); do curl -sf http://localhost:8123/api/states && break || sleep 1; done`
  - **Verify**: `curl -sf http://localhost:8123/api/states && echo "VE1_PASS"`
  - **Done when**: HA instance running and responding
  - **Commit**: None

- [ ] 5.5 VE2 [VERIFY] E2E check: test panel loads with real data
  - **Do**:
    1. Navigate to panel URL in Playwright: `await page.goto('/panel/ev-trip-planner-test-vehicle')`
    2. Verify panel shows correct vehicle ID
    3. Verify trips load
    4. Verify sensors display with values
  - **Verify**: `grep -r "EV Trip Planner -" tests/e2e/`
  - **Done when**: Panel displays correctly with real HA data
  - **Commit**: None

- [ ] 5.6 VE3 [VERIFY] E2E cleanup: stop test environment
  - **Do**:
    1. Stop HA instance: `docker-compose down`
    2. Cleanup test artifacts
  - **Verify**: `! docker ps -q | grep home-assistant`
  - **Done when**: No test processes running
  - **Commit**: None
