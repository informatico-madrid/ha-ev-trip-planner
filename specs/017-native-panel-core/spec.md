...
El testing E2E con Playwright sigue este flujo iterativo hasta lograr el panel nativo:

1. **Login**: Acceder a Home Assistant con credenciales (usuario: malka, contraseña: Darkpunk666/)
2. **Navegación**: Ir a `Configuración > Integraciones`
3. **Agregar Integración**: Buscar y agregar "EV Trip Planner"
4. **Configurar Vehículo**: Crear un nuevo vehículo con datos de prueba
5. **Verificación**: Comprobar si se creó el panel nativo en el sidebar
6. **Debugging**: Si no se crea el panel:
   - Verificar errores en Playwright console logs
   - Verificar errores en los logs de Home Assistant
   - Identificar y corregir el problema
7. **Iterar**: Repetir desde el paso 3 hasta lograr el panel funcionando

### Herramientas de Debugging

- **Playwright Console Logs**: `page.on('console', ...)` para capturar logs del navegador
- **Page Errors**: `page.on('pageerror', ...)` para capturar errores de JavaScript
- **Screenshots**: `page.screenshot()` para capturar estado de la página
- **HA API**: Consultar `/api/states` y `/api/services` para verificar entidades y servicios
- **HA Logs**: Verificar logs de Home Assistant para errores de integración

### Criterios de Éxito del Test

- [ ] El panel aparece en el sidebar de Home Assistant
- [ ] El panel carga sin errores de JavaScript
- [ ] El panel muestra información del vehículo
- [ ] No hay errores en los logs de HA
