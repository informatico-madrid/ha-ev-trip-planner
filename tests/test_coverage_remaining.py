...
class TestCalculateNextRecurringDatetimeExceptionPath:
    ...
    async def test_weekly_trip_exception_in_calculate_next_recurring_datetime(self):
        ...
        with patch.object(tm, '_get_all_active_trips', return_value=[
            {
                "id": "rec_monday_123",
                "tipo": "recurring",
                "dia_semana": "lunes",
...
class TestHourlyRefreshExceptionHandler:
    ...
    async def test_hourly_refresh_callback_exception_is_caught(self):
        ...
        """
        from custom_components.ev_trip_planner import _hourly_refresh_callback, EVTripRuntimeData
        from custom_components.ev_trip_planner.trip_manager import TripManager
        from datetime import datetime
        
        # Create mock trip manager that raises exception
        mock_trip_manager = AsyncMock(spec=TripManager)
        mock_trip_manager.publish_deferrable_loads = AsyncMock(side_effect=Exception("Test exception"))
        ...
        with patch.object(logging.getLogger("custom_components.ev_trip_planner"), "warning") as mock_warning:
            now = datetime.now()
