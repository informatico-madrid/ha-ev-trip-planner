...
def test_charging_status_sensor_entity_translation_exists():
    ...
    assert translations["entity"]["charging_status_sensor"]["name"] != "charging_status_sensor"
    assert translations["entity"]["charging_status_sensor"]["name"] != "charging status sensor"
...
def test_planning_sensor_entity_translation_clear():
    ...
    if planning_key in translations:
        ...
        assert "planificación" not in translation.lower() or "emhass" in translation.lower()
...
def test_strings_json_has_clear_planning_sensor_description():
    ...
    planning_desc = strings["config"]["step"]["emhass"]["data_description"]["planning_sensor_entity"]
    
    # Should mention EMHASS or deferrable loads, not just "planning"
    assert "emhass" in planning_desc.lower() or "diferible" in planning_desc.lower() or "deferrable" in planning_desc.lower()
    assert "planificación" not in planning_desc.lower() or "emhass" in planning_desc.lower()
...
def test_spanish_translation_has_clear_planning_sensor_description():
    ...
    planning_desc = translations["config"]["step"]["emhass"]["data_description"]["planning_sensor_entity"]
