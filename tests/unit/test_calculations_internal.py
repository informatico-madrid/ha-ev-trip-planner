...
#
# Tests for deficit._calculate_soc_at_trip_starts (29 survivors)
#


class TestCalculateSocAtTripStarts:
    """Tests for deficit.calculate_soc_at_trip_starts.

    Direct tests for SOC calculation at trip starts.
    """

    def test_empty_inputs_return_empty(self):
        """Empty trips or windows returns empty list."""
        from custom_components.ev_trip_planner.calculations.deficit import (
            calculate_soc_at_trip_starts,
        )

        assert calculate_soc_at_trip_starts([], 50.0, [], 3.6, 75.0) == []

    def test_single_trip_soc_increases(self):
        """Single trip: arrival SOC > start SOC after charging."""
        from custom_components.ev_trip_planner.calculations.deficit import (
            calculate_soc_at_trip_starts,
        )

        windows = [{"ventana_horas": 2.0, "kwh_necesarios": 5.0}]
        ...
        assert len(result) == 1
        assert result[0]["soc_inicio"] == 40.0
        # 2h * 3.6kW = 7.2 kWh, at 75kWh battery = 9.6% SOC increase
        assert result[0]["arrival_soc"] > result[0]["soc_inicio"]
        assert result[0]["arrival_soc"] <= 100.0

    def test_chained_trips_soc_chains(self):
        """Multiple trips: arrival SOC of trip N = start SOC of trip N+1."""
        from custom_components.ev_trip_planner.calculations.deficit import (
            calculate_soc_at_trip_starts,
        )

        windows = [
            {"ventana_horas": 2.0, "kwh_necesarios": 5.0},
            {"ventana_horas": 1.0, "kwh_necesarios": 3.0},
        ]
        result = calculate_soc_at_trip_starts(
            trips=[{"id": "t1"}, {"id": "t2"}],
            soc_inicial=50.0,
        ...
        assert len(result) == 2
        # Trip 1: 50% + (2h * 3.6kW / 75kWh * 100) = 50% + 9.6% = 59.6%
        assert abs(result[0]["soc_inicio"] - 50.0) < 0.01
        assert result[0]["arrival_soc"] == result[1]["soc_inicio"]
        # Trip 2: 59.6% + (1h * 3.6kW / 75kWh * 100) = 59.6% + 4.8% = 64.4%
        assert result[1]["arrival_soc"] > result[1]["soc_inicio"]

    def test_soc_clamped_at_100(self):
        """SOC never exceeds 100% even with excessive charging."""
        from custom_components.ev_trip_planner.calculations.deficit import (
            calculate_soc_at_trip_starts,
        )

        windows = [{"ventana_horas": 10.0, "kwh_necesarios": 5.0}]
        result = calculate_soc_at_trip_starts(
            trips=[{"id": "t1"}],
            soc_inicial=98.0,
            windows=windows,
        ...
        assert len(result) == 1
        # Would charge 110 kWh (impossible) but clamped to 100%
        assert result[0]["arrival_soc"] <= 100.0

    def test_zero_charging_power_no_soc_increase(self):
        """Zero charging power results in no SOC increase."""
        from custom_components.ev_trip_planner.calculations.deficit import (
            calculate_soc_at_trip_starts,
        )
    ...
    def test_zero_battery_capacity_no_soc_increase(self):
        """Zero battery capacity results in no SOC increase."""
        from custom_components.ev_trip_planner.calculations.deficit import (
            calculate_soc_at_trip_starts,
        )
    ...
    def test_result_has_required_keys(self):
        """Each result dict has required keys: soc_inicio, trip, arrival_soc."""
        from custom_components.ev_trip_planner.calculations.deficit import (
            calculate_soc_at_trip_starts,
        )
        ...
        assert isinstance(result[0]["soc_inicio"], (int, float))
        assert isinstance(result[0]["arrival_soc"], (int, float))
