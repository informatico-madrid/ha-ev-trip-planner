...
class TestSOCMixinExecution:
    ...
    def test_parse_trip_datetime_exception(self):
        ...
        soc = _make_soc()
        result = soc._parse_trip_datetime("not-a-date!!")
        # Default allow_none=False → returns datetime.now(timezone.utc)
        assert result is not None

    def test_parse_trip_datetime_allow_none(self):
        """Invalid trip datetime with allow_none → returns None."""
        soc = _make_soc()
        result = soc._parse_trip_datetime("not-a-date!!", allow_none=True)
    ...
    async def test_calcular_ventana_carga_next_trip_none(self):
        ...
        result = await soc.calcular_ventana_carga(
            trip={"tipo": "punctual"},
            soc_actual=50.0,
            hora_regreso=future,
            charging_power_kw=3.6,
    ...
    async def test_calcular_ventana_carga_multitrip_with_trip_tipo_datetime(self):
        ...
        result = await soc.calcular_ventana_carga_multitrip(
            trips,
            soc_actual=50.0,
            hora_regreso=future,
            charging_power_kw=3.6,
