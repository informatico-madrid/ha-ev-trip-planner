...
class TestConfigFlowIssues:
    ...
    def test_vehicle_type_not_in_config_flow(self):
        ...
        assert "vehicle_type" not in schema_str, \
        ...
        assert "vehicle_name" in schema_str, \
    ...
    def test_config_flow_step_count(self):
        ...
        for step_name in step_schemas:
            assert hasattr(config_flow, step_name), \
    ...
    def test_charging_status_sensor_spanish_translation(self):
        ...
        assert "entity" in es_translations, "Missing 'entity' section in translations/es.json"
        assert "charging_status_sensor" in es_translations["entity"], \
        ...
        assert spanish_name == "Sensor de Estado de Carga del Vehículo", \
        ...
        assert "Charging Status Sensor" not in spanish_name, \
    ...
    def test_charging_status_config_flow_spanish(self):
        ...
        assert spanish_label == "Estado de Carga (opcional)", \
        ...
        assert "charging_status" in sensors_desc, \
        ...
        assert "opcional" in spanish_description.lower(), \
            "data_description should be in Spanish"
        assert "sensor binario" in spanish_description.lower(), \
    ...
    def test_charging_status_sensor_has_help_hint(self):
        ...
        assert "opcional" in hint.lower(), \
            "Hint should mention it's optional"

        # Check 2: Mention it's a binary sensor
        assert "sensor binario" in hint.lower() or "binario" in hint.lower(), \
            "Hint should mention binary sensor"

        # Check 3: Explain what state to look for (on/charging)
        assert "on" in hint.lower() or "cargando" in hint.lower(), \
        ...
        assert "charging" in hint.lower() or "charge" in hint.lower() or "plugged" in hint.lower(), \
        ...
        assert "binary_sensor" in hint.lower(), \
