...
async def test_power_profile_without_vehicle_config(trip_manager, sample_trip):
    ...
    """
    # Add trip to state (new composition architecture)
    trip_manager._state.punctual_trips = {"trip_1": sample_trip}

    # Call without vehicle_config (should not crash)
    profile = await trip_manager._power.async_generate_power_profile(
