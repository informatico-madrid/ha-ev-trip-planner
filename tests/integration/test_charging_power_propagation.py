...
class TestChargingPowerKwSensorIntegration:
    ...
    @pytest.mark.asyncio
    async def test_power_profile_shows_correct_charging_power(self, mock_hass):
        ...
        print(
            f"After options update: adapter._charging_power_kw = {adapter._charging_power_kw}"
        )
        print(
            f"After options update: LoadPublisher.charging_power_kw = {adapter._load_publisher.charging_power_kw}"
        )
        ...
        actual_power_watts = (
            adapter._load_publisher.charging_power_kw * 1000
        )  # 3400W (FIXED!)

        assert expected_power_watts == 3400, (
            f"Expected 3400W, got {expected_power_watts}"
        )
