...
async def test_p_deferrable_matrix_single_trip_def_start_0(
    ...
    print(
        f"Filled positions: {filled_positions[:10]}..."
        if len(filled_positions) > 10
        else f"Filled positions: {filled_positions}"
    )
    ...
    for t in range(0, charging_start):
        assert row[t] == 0, (
            f"Position {t} should be 0 (before charging window) but is {row[t]}"
        )
    ...
    for t in range(charging_end, len(row)):
        assert row[t] == 0, (
            f"Position {t} should be 0 (after charging window) but is {row[t]}"
        )
