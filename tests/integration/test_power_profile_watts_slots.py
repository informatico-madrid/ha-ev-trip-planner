...
async def test_power_profile_watts_single_trip_compacted_at_end(
    ...
    print(
        f"power_profile filled positions: {filled_positions[:10]}..."
        if len(filled_positions) > 10
        else f"power_profile filled positions: {filled_positions}"
    )
    ...
    for t in range(charging_start, charging_end):
        assert row[t] == power_watts, (
            f"matrix row[{t}] should be {power_watts} (charging power) but is {row[t]}"
        )
    ...
    print(
        f"✓ matrix row correctly compacted to {def_total} slots at positions [{charging_start}, {charging_end})"
    )
    print(f"✓ All {charging_start} positions before charging window are 0")
    print(
        f"✓ All {len(power_profile) - charging_end} positions after charging window are 0"
    )
