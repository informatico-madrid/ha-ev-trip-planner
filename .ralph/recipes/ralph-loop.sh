...
export RALPH_WORKER_MODEL="MiniMax-M2.5"
export RALPH_REVIEWER_PROVIDER="github_copilot"
export RALPH_REVIEWER_MODEL="gpt-5-mini"
export CLAUDE_CODE_COMMAND="claude"
export RALPH_WORKER_PROVIDER="claude-code"
export RALPH_MAX_ITERATIONS=500
export RALPH_RECIPE_DIR=".ralph/recipes"
set -e
...
for i in $(seq 1 "$MAX_ITERATIONS"); do
    ...
    # Mostrar un resumen de lo que produjo el worker en esta iteración
    echo ""
    echo -e "${BLUE}--- Resumen del Worker (estado: $STATE_DIR)${NC}"
    if [ -f "$STATE_DIR/work-summary.txt" ]; then
        echo -e "${GREEN}Work Summary:${NC}"
        sed -n '1,200p' "$STATE_DIR/work-summary.txt" || true
    else
        echo -e "${YELLOW}No se encontró .goose/ralph/work-summary.txt${NC}"
    fi
    if [ -f "$STATE_DIR/work-complete.txt" ]; then
        echo -e "${GREEN}Work Status:${NC} COMPLETE"
        cat "$STATE_DIR/work-complete.txt" 2>/dev/null || true
    else
        echo -e "${YELLOW}Work Status:${NC} INCOMPLETE"
    fi
    ...
    # Mostrar decisión y feedback del revisor
    echo ""
    echo -e "${BLUE}--- Resumen del Revisor (estado: $STATE_DIR)${NC}"
    if [ -f "$STATE_DIR/review-result.txt" ]; then
        echo -e "${GREEN}Review Result:${NC}"
        sed -n '1,20p' "$STATE_DIR/review-result.txt" || true
    else
        echo -e "${YELLOW}No se encontró .goose/ralph/review-result.txt${NC}"
    fi
    if [ -f "$STATE_DIR/review-feedback.txt" ]; then
        echo -e "${GREEN}Review Feedback:${NC}"
        sed -n '1,200p' "$STATE_DIR/review-feedback.txt" || true
    fi
    echo -e "${BLUE}Archivos en $STATE_DIR:${NC}"
    ls -la "$STATE_DIR" | sed 's/^/  /'
    echo ""
